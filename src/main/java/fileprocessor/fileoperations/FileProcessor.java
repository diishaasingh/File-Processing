        ...
        if (files != null) {
            ...
            for (File file : files) {
                if (file.getName().endsWith(".trg")) {
                    ...
                    processCsvFile(file, destinationFolder);
                } else if (file.getName().matches(".*\\.(txt|other_extension)")) {
                    csvOrTxtFileFound = true;
                    processDataFile(file, destinationFolder);
                }
            }

            if (!csvOrTxtFileFound) {
                throw new FileNotFoundException("No .csv or .txt file found in the source folder.");
            }
        } else {
